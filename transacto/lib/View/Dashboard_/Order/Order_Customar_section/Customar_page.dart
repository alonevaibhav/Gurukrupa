...
import 'package:transacto/View/Dashboard_/Order/Order_Customar_section/Update_section.dart';
...
class _CustomerListPageState extends State<CustomerListPage> {
  ...
  List<Map<String, dynamic>> customerList = [
    {'name': 'Vaibhav Alone', 'phone': '1234567890', 'address': 'Dhaka', 'date': '2024-10-01'},
  ...
  void updateFilteredList() {
    setState(() {
      filteredCustomerList = customerList.where((customer) {
        final matchesSearch = customer['name'].toLowerCase().contains(searchQuery.toLowerCase());
  ...
  Widget build(BuildContext context) {
    ...
    endIndex = endIndex > filteredCustomerList.length ? filteredCustomerList.length : endIndex;
    List<Map<String, dynamic>> currentCustomers = filteredCustomerList.sublist(startIndex, endIndex);
        ...
        child: Column(
          children: [
            ...
            Row(
              ...
              children: [
                _buildActionButton(Icons.add, 'Add Orders', Colors.orange, Colors.white, isMobile),
              ],
            ),
  ...
  Widget _buildSearchAndFilterRow(bool isMobile, bool isTablet, bool isDesktop) {
    return isDesktop
        ? Row(
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        columns: [
          ...
          DataColumn(
            label: _buildConstrainedHeader('Customer Name', minWidth: 150, maxWidth: 300),
          ),
          DataColumn(
            label:  _buildConstrainedHeader('Phone Number', minWidth: 100, maxWidth: 400),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Address', minWidth: 100, maxWidth: 300),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Date', minWidth: 100, maxWidth: 300),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Action', minWidth: 10, maxWidth: 300),
          ),
        ],
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => UpdateOrderCustomerPage(
                              onAddCustomer: (Map<String, dynamic> customerData) {
                                // Handle the added/updated customer data here
                                print('Customer Added/Updated: $customerData');
                                // Add your logic: Save to a database, update UI, etc.
                              }, existingCustomer: {},
                            ),
                          ),
                        );
...
// Helper function to build constrained headers
  Widget _buildConstrainedHeader(String text, {double minWidth = 130, double maxWidth = 230}) {
...
// Helper function to build wrapped text cells for long content
  Widget _buildWrappedTextCell(String text, {int maxLines = 3}) {
    return ConstrainedBox(
      constraints: BoxConstraints(minWidth: 150, maxWidth: 300), // Adjust column width
      child: Text(
        text,
...
// Helper function to build general text cells
  Widget _buildTextCell(String text) {
    return ConstrainedBox(
      constraints: BoxConstraints(minWidth: 130, maxWidth: 230), // General width constraints
      child: Text(
        text,
        style: TextStyle(fontSize: 16),
...
// Helper function to build constrained cells for non-wrapped content
  Widget _buildConstrainedCell(String text, {double minWidth = 130, double maxWidth = 230}) {
    return ConstrainedBox(
      constraints: BoxConstraints(minWidth: minWidth, maxWidth: maxWidth),
      child: Text(
        text,
  ...
  Widget _buildActionButton(IconData icon, String text, Color bgColor, Color textColor, bool isMobile) {
          ...
          builder: (BuildContext context) {
            return AddOrderCustomerPage(
              onAddCustomer: (Map<String, dynamic> customerData) {
                ...
                setState(() {
                  customerList.add(customerData); // Add the new customer to the list
