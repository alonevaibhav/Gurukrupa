...
//
// class AmountReservedSection extends StatefulWidget {
//   final String customerName;
//
//   const AmountReservedSection({Key? key, required this.customerName})
//       : super(key: key);
//
//   @override
//   _AmountReservedSectionState createState() => _AmountReservedSectionState();
// }
//
// class _AmountReservedSectionState extends State<AmountReservedSection> {
...
//                     filteredCustomerList[customerIndex]
...
//       int index, String name, String order, String address, String date) {
//     return Row(
//       children: [
//         Expanded(
//           child: Align(
...
//                                           (customer) =>
//                                       customer['amount'] == name &&
//                                           customer['order'] == order &&
//                                           customer['date'] == date);
//                                   if (actualIndex != -1) {
//                                     customerList.removeAt(
//                                         actualIndex); // Remove customer from the list
  ...
  Widget _buildDesktopCustomerTable(List<Map<String, dynamic>> customers) {
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        columnSpacing: 80, // General space between columns
        columns: [
          DataColumn(
            label: _buildConstrainedHeader('Sr.No', minWidth: 50, maxWidth: 60),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Account Name',
                minWidth: 50, maxWidth: 200),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Transaction ID',
                minWidth: 50, maxWidth: 200),
          ),
          DataColumn(
            label: _buildConstrainedHeader('Amount Received',
                minWidth: 50, maxWidth: 200),
          ),
          DataColumn(
          ...
          DataColumn(
            label: _buildConstrainedHeader('Payment Type',
                minWidth: 10, maxWidth: 200),
          ),
          DataColumn(
            label:
                _buildConstrainedHeader('Action', minWidth: 10, maxWidth: 200),
          ),
        ],
        rows: List.generate(itemsPerPage, (index) {
          if (index < customers.length) {
            ...
            return DataRow(
              cells: [
                DataCell(_buildConstrainedCell(
                    ...
                    minWidth: 50,
                    maxWidth: 60)),
                DataCell(_buildWrappedTextCell(customer['name'],
                    maxLines: 3)), // Wrap for long names
                DataCell(_buildTextCell(customer['phone'])),
                DataCell(_buildWrappedTextCell(customer['address'],
                    maxLines: 3)), // Wrap for long addresses
                DataCell(_buildTextCell(customer['date'])),
                DataCell(_buildTextCell(customer['type'])),
                DataCell(
                  Row(
            ...
            // Empty row logic
            return DataRow(
              cells: List.generate(7, (index) => DataCell(SizedBox.expand())),
            );
          }
        }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                Text('Account Name: ${customer['name']}'),
                SizedBox(height: 4),
                Text('Transaction ID: ${customer['phone']}'),
                SizedBox(height: 4),
                Text('Amount Received: ${customer['address']}'),
                SizedBox(height: 4),
                Text('Date: ${customer['date']}'),
                Text('Payment Type: ${customer['type']}'),
                SizedBox(height: 8),
                Row(
                  mainAxisAlignment: MainAxisAlignment.end,
                  children: [
